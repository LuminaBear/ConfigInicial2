...
void Animation() {
	...
	if (AnimDog) {
		// Animación de las patas (Movimiento natural)
		// Se ejecuta siempre que el perro esté "caminando" en cualquier estado
		if (dogAnim > 0) {
			if (!step) {
				RLegs += 1.5f;
				FLegs += 1.5f;
		...
		case 1:
			dogRot = 0.0f;
			dogPos.z += speed;
			if (dogPos.z >= limit) dogAnim = 2;
			break;

		case 2:
			dogRot = -90.0f;
			dogPos.x -= speed;
			if (dogPos.x <= -limit) dogAnim = 3;
			break;

		case 3:
			dogRot = 180.0f;
			dogPos.z -= speed;
			if (dogPos.z <= -limit) dogAnim = 4;
			break;

		case 4:
			dogRot = -270.0f;
			dogPos.x += speed;
			if (dogPos.x >= limit) dogAnim = 5;
			break;
		...
		case 5:
			...
			// Si llega cerca del origen, reinicia el ciclo
